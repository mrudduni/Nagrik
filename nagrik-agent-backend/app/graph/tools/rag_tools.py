...
@tool
def tree_rag_search(query: str, ministry: str | None = None, department: str | None = None) -> dict:
    """Search government scheme/policy knowledge base. Optionally filter
    by ministry/department for more precise hierarchical retrieval.
    Returns matched chunks with source citations, or an explicit
    'not found' result rather than a fabricated answer."""
    return _tree_rag_search(query=query, ministry=ministry, department=department)
